        ...
        <div className="flex items-center gap-4">
          ...
          <div>
            ...
            <p className="text-muted-foreground">
              {jobWorkflow.jobNumber} - {jobWorkflow.customer.businessName || jobWorkflow.customer.contactPerson}
            </p>
          </div>
        </div>
            ...
            <CardContent>
              <div className="space-y-4">
                {workflow.timeline.map((update: any, idx: number) => (
                  <div key={idx} className="flex items-start gap-4 border-l-2 border-primary pl-4 pb-4">
          ...
          <Card>
            ...
            <CardContent>
              {workflow.requisitions.length === 0 ? (
                <p className="text-center text-muted-foreground py-8">No requisitions submitted</p>
              ) : (
                <div className="space-y-4">
                  {workflow.requisitions.map((req: any) => (
                    <Card key={req.id} className="border">
                      <CardHeader className="pb-3">
                        <div className="flex items-center justify-between">
                          <div>
                            <p className="font-mono font-medium">{req.requisitionNumber}</p>
                            <p className="text-sm text-muted-foreground">
                              Requested by {req.technician.user.firstName} {req.technician.user.lastName} •{' '}
                      ...
                      <CardContent>
                        ...
                        {req.notes && (
                          <div className="mt-3 p-3 bg-muted rounded-lg">
                            <p className="text-sm"><strong>Notes:</strong> {req.notes}</p>
                          </div>
                        )}
                      </CardContent>
                    </Card>
            ...
            <CardContent>
              {workflow.preInspectionChecklist.length === 0 ? (
                ...
                <Table>
                  ...
                  <TableBody>
                    {workflow.preInspectionChecklist.map((check: any, idx: number) => (
          ...
          <Card>
            ...
            <CardContent>
              <div className="grid grid-cols-2 gap-6">
                <div>
                  <h3 className="font-semibold mb-2">IMEI Numbers</h3>
                  {jobWorkflow.imeiNumbers && jobWorkflow.imeiNumbers.length > 0 ? (
                    <div className="space-y-1">
                      {jobWorkflow.imeiNumbers.map((imei: string, idx: number) => (
                        <p key={idx} className="font-mono text-sm">{imei}</p>
                      ))}
                    </div>
                  ) : (
                ...
                <div>
                  <h3 className="font-semibold mb-2">SIM Card</h3>
                  <p className="font-mono text-sm">{jobWorkflow.simCardIccid || 'Not provided'}</p>
                </div>
                <div>
                  <h3 className="font-semibold mb-2">MAC Address</h3>
                  <p className="font-mono text-sm">{jobWorkflow.macAddress || 'Not provided'}</p>
                </div>
                <div>
                  <h3 className="font-semibold mb-2">GPS Coordinates</h3>
                  <p className="text-sm">{jobWorkflow.gpsCoordinates || 'Not captured'}</p>
                </div>
                <div className="col-span-2">
                  <h3 className="font-semibold mb-2">Installation Notes</h3>
                  <div className="p-3 bg-muted rounded-lg">
                    <p className="text-sm">{jobWorkflow.installationNotes || 'No notes provided'}</p>
                  </div>
                </div>
                <div className="col-span-2">
                  <h3 className="font-semibold mb-2">Installation Photos</h3>
          ...
          <Card>
            ...
            <CardContent>
              {workflow.postInspectionChecklist.length === 0 ? (
                <p className="text-center text-muted-foreground py-8">No post-inspection data</p>
              ) : (
                <Table>
                  <TableHeader>
                    <TableRow>
                  ...
                  <TableBody>
                    {workflow.postInspectionChecklist.map((check: any, idx: number) => (
                      <TableRow key={check.id}>
                        <TableCell>{idx + 1}</TableCell>
                        <TableCell className="font-medium">{check.checklistItem.name}</TableCell>
                        <TableCell>
