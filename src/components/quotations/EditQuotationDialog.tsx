...
import { quotationsService, Quotation, UpdateQuotationRequest } from '@/api/services/quotations.service';
...
const EditQuotationDialog: React.FC<EditQuotationDialogProps> = ({
  ...
  useEffect(() => {
    if (quotation && open) {
      ...
      // Convert existing items safely
      const items: QuotationItem[] = (quotation.items || []).map(item => ({
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const updatedQuotation = await quotationsService.update(quotation.id, requestData);
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            ...
            <div>
              ...
              <div className="p-2 bg-muted rounded-md">
                <p className="font-medium">{quotation.customer?.businessName || 'N/A'}</p>
                <p className="text-sm text-muted-foreground">{quotation.customer?.contactPerson || 'N/A'}</p>
              </div>
            </div>
                      ...
                      >
                        <div className="flex justify-between items-center">
                          ...
                          <div className="text-right">
                            <p className="font-medium">{formatCurrency(Number(product.sellingPrice) || 0)}</p>
                          </div>
                        </div>
                      </div>
                    ))}
