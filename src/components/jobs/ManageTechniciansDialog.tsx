...
import { Loader2, UserPlus, UserMinus, Star, MapPin, Phone, AlertCircle } from 'lucide-react';
import { toast } from 'sonner';
import { jobsService, Job, AddTechnicianRequest } from '@/api/services/jobs.service';
...
const ManageTechniciansDialog = ({ open, onOpenChange, job }: ManageTechniciansDialogProps) => {
  ...
  const [techToSetPrimary, setTechToSetPrimary] = useState<{ id: string; name: string } | null>(null);
  ...
  const addMutation = useMutation({
    mutationFn: (data: AddTechnicianRequest) => jobsService.addTechnician(job!.id, data),
    onSuccess: () => {
  ...
  const setPrimaryMutation = useMutation({
    mutationFn: (techId: string) => jobsService.setPrimaryTechnician(job!.id, techId),
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ['jobs'] });
    ...
    onError: (error: any) => {
      toast.error(error.response?.data?.message || 'Failed to set primary technician');
    },
  });
  ...
  const availableTechnicians = technicians.filter((t) => !assignedTechIds.includes(t.id));
  ...
  return (
    <>
      <Dialog open={open} onOpenChange={onOpenChange}>
        <DialogContent className="max-w-3xl max-h-[90vh] overflow-y-auto">
          <DialogHeader>
            ...
            <DialogDescription>
              Add or remove technicians, and set the primary technician for this job.
            </DialogDescription>
          </DialogHeader>
          ...
          <div className="space-y-6 py-4">
            ...
            <div>
              ...
              {currentTechnicians.length === 0 ? (
                <Alert>
                  ...
                  <AlertDescription>No technicians assigned yet</AlertDescription>
                </Alert>
              ) : (
            ...
            <div>
              ...
              {availableTechnicians.length === 0 ? (
                <Alert>
                  <AlertCircle className="h-4 w-4" />
                  <AlertDescription>No additional available technicians found</AlertDescription>
                </Alert>
              ) : (
      ...
      <AlertDialog open={!!techToRemove} onOpenChange={() => setTechToRemove(null)}>
        <AlertDialogContent>
          <AlertDialogHeader>
            <AlertDialogTitle>Remove Technician</AlertDialogTitle>
            <AlertDialogDescription>
              Are you sure you want to remove <strong>{techToRemove?.name}</strong> from this job?
            </AlertDialogDescription>
          </AlertDialogHeader>
          <div className="py-4">
      ...
      <AlertDialog open={!!techToSetPrimary} onOpenChange={() => setTechToSetPrimary(null)}>
        <AlertDialogContent>
          <AlertDialogHeader>
            <AlertDialogTitle>Set Primary Technician</AlertDialogTitle>
            <AlertDialogDescription>
              Are you sure you want to set <strong>{techToSetPrimary?.name}</strong> as the primary technician?
              This will replace the current primary technician.
            </AlertDialogDescription>
          </AlertDialogHeader>
          <AlertDialogFooter>
            <AlertDialogCancel>Cancel</AlertDialogCancel>
            <AlertDialogAction onClick={handleSetPrimary} disabled={setPrimaryMutation.isPending}>
              {setPrimaryMutation.isPending && <Loader2 className="mr-2 h-4 w-4 animate-spin" />}
              Confirm
            </AlertDialogAction>
          </AlertDialogFooter>
        </AlertDialogContent>
